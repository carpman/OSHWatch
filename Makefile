...
SRCS = debug.c lsm9ds0.c bt.c hal.c display.c main.c system_stm32f0xx.c
...
CFLAGS  = -Wall -g -std=c99 -Os -DDEBUG
...
debug: $(PROJ_NAME).elf
	xterm -e st-util &
	sleep 1
	arm-none-eabi-gdb --eval-command="target remote localhost:4242"  $(PROJ_NAME).elf -ex 'load'
